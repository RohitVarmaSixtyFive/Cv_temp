...
class ModelBuilder:
    ...
    def build(self):
        model = ViTForClassfication(self.config)
...
class ModelTrainer:
    ...
    def __init__(self, model, config, train_loader, val_loader, test_loader, run_name=None):
        ...
        self.run_name = f"ViT-Run-{config['patch_size']}-{config['hidden_size']}-{config['num_attention_heads']}-{config['num_hidden_layers']}-{config['intermediate_size']}-{config['positional_embedding']}"
        self.wandb_run = None
