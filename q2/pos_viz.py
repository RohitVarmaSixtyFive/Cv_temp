...
import argparse
import sys
from ViT import DiffViT as ViTForClassfication
# import seaborn as sns

def visualize_positional_embedding(model):

    pos_embed = model.embedding.position_embedding.position_embeddings.detach().cpu()
    ...
    for i, (pos, name) in enumerate(zip(sample_positions, sample_names)):
        ...
        # Plot
        im = axes[i].imshow(sim_map, cmap='viridis')
    ...
    if len(axes) > len(sample_positions):
        im = axes[len(sample_positions)].imshow(similarity, cmap='viridis')
